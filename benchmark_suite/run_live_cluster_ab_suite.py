...
on the actual live running Ceph cluster (1 MON, 1 MGR, 1 OSD BlueStore, Beast RGW).
"""
...
def restart_rgw(scheduler_type="dmclock", per_tenant="true", adaptive="false"):
    print(f"\n[*] Switching live Ceph RGW to: scheduler={scheduler_type}, per_tenant={per_tenant}, adaptive={adaptive}...")
    
    # 1. Kill existing radosgw
    ...
    # 2. Start radosgw with target scheduler config
    cmd = [
        RGW_BIN,
        "-c", CEPH_CONF,
        f"--log-file={LOG_FILE}",
...
# Workload Definitions for Live Cluster
...
def format_ab_table(title, throttler_res, dmclock_res):
    ...
    print("="*110)
    header = f"| {'Tenant Persona':<26} | {'Throttler TPS':<14} | {'Throttler p50':<14} | {'dmClock TPS':<14} | {'dmClock p50':<14} | {'TPS Ratio':<10} |"
    sep = "+" + "-"*28 + "+" + "-"*16 + "+" + "-"*16 + "+" + "-"*16 + "+" + "-"*16 + "+" + "-"*12 + "+"
    print(sep)
    print(header)
    print(sep)
    ...
    for name in t_map:
        ...
        row = f"| {name:<26} | {t_tps:<14.1f} | {t['p50_ms']:<12.1f}ms | {d_tps:<14.1f} | {d['p50_ms']:<12.1f}ms | {ratio:<10} |"
        print(row)
    print(sep)

def main():
    print("#"*110)
    print(" STARTING RIGOROUS LIVE CEPH CLUSTER A/B BENCHMARK (THROTTLER VS DMCLOCK)")
    print("#"*110)
    ...
    print("\n\n" + "#"*110)
    print(" FINAL LIVE CEPH CLUSTER A/B COMPARATIVE RESULTS")
    print("#"*110)
    ...
    with open(out_file, "w") as f:
        f.write("# Live Ceph Cluster A/B Benchmark Report: Throttler vs dmClock\n\n")
        f.write(f"Executed on actual running Ceph cluster with real BlueStore OSD and Beast RGW.\n\n")
        for name, _, _ in workloads:
